...
const { Client, types, geometry, ExecutionProfile, datastax } = require('cassandra-driver');
const helper = require('../helper');
const vdescribe = helper.vdescribe;
const { InetAddress, Uuid, Long, Tuple } = types;
const { Point, LineString, Polygon } = geometry;
const { asUdt, asInt } = datastax.graph;
const dseGraph = require('../../index');
const tinkerpop = require('gremlin');
const P = tinkerpop.process.P;
const __ = tinkerpop.process.statics;
const { VertexProperty, Vertex } = tinkerpop.structure;
...
vdescribe('5.0', 'DseGraph', function () {
  ...
  context('with classic graph engine', () => {
    ...
    describe('queryFromTraversal()', function() {
      it('should produce a query string usable with Client#executeGraph()', wrapClient(function (client, done) {
        ...
        return g.V().hasLabel('person').has('name', 'marko').as('a')
          ...
          .toList().then(results => {
            ...
            results.forEach(path => {
              ...
              assert.strictEqual(knows.outV.label, 'person');
              assert.deepEqual(knows.outV.id, marko.id);
              assert.strictEqual(knows.inV.label, 'person');
              assert.deepEqual(knows.inV.id, josh.id);
              ...
              assert.strictEqual(created.outV.label, 'person');
              assert.deepEqual(created.outV.id, josh.id);
              assert.strictEqual(created.inV.label, 'software');
              assert.deepEqual(created.inV.id, software.id);
      ...
      describe('Traversal#toList()', function () {
        it('should return an Array of traversers', function () {
          return g.V().hasLabel('person').toList().then(people => {
            ...
            people.forEach(v => {
              helper.assertInstanceOf(v, Vertex);
    ...
    vdescribe('6.0', 'queryFromBatch()', function () {
      it('should produce a query usable with Client#executeGraph()', wrapClientPromise(function (client) {
        const g = dseGraph.traversalSource(client);
  ...
  vdescribe('6.8', 'with core graph engine', () => {
    ...
    before(() => {
      const createLabelQuery = `
        schema.vertexLabel('label_test').ifNotExists().partitionBy('id', Int)
          ...
          .create();
        schema.edgeLabel('relates').
          from('label_test').to('label_test').
          create()`;
    ...
    before(() => {
      const createSchemaQuery = `
          ...
          schema.vertexLabel('label_complex').ifNotExists().partitionBy('id', Int)
            ...
            .property('prop_list1', listOf(Text))
    ...
    before(() => client.execute(
      `INSERT INTO ${graphName}.label_complex (id, prop_tuple1) VALUES (1000, (10, 'ten'))`));

    before(() => client.execute(
      `INSERT INTO ${graphName}.label_complex
           (id, prop_udt1) VALUES (2000, {address1: 'Apollo St.', zip_code: 12345 })`));

    after(() => client.shutdown());

    describe('queryFromBatch()', () => {
      it('should produce a query usable with Client#executeGraph()', async () => {
        const g = dseGraph.traversalSource(client, { graphName });

        const id1 = idSequence++;
        const id2 = idSequence++;

        const batch = [
          g.addV('label_test').property('prop_text', 'Matt').property('id', id1),
          g.addV('label_test').property('prop_text', 'Olivia').property('id', id2),
          g.V().has('label_test', 'id', id1).addE('relates').to(__.V().has('label_test', 'id', id2))
        ];

        const query = dseGraph.queryFromBatch(batch);

        await client.executeGraph(query, null, { graphName });
        const names = await g.V().has('label_test', 'id', id1).out('relates').values('prop_text').toList();
        assert.deepEqual(names, ['Olivia']);
      });
    });
    ...
    describe('traversalSource()', function () {
      ...
      it('should execute a simple traversal', async () => {
        const result = await g.V().hasLabel('label_test').count().toList();
        helper.assertInstanceOf(result, Array);
        assert.strictEqual(result.length, 1);
        const count = result[0];
        helper.assertInstanceOf(count, Long);
      });

      it('should support tuple', async () => {
        const id = idSequence++;
        const value = new Tuple(asInt(1), 'one');
        await g.addV('label_complex').property('id', id).property('prop_tuple1', value).iterate();

        const result = await g.V().hasLabel('label_complex').has('id', id).values('prop_tuple1').toList();
        helper.assertInstanceOf(result, Array);
        const actual = result[0];
        helper.assertInstanceOf(actual, Tuple);
        assert.strictEqual(actual.elements[0], value.elements[0].value);
        assert.strictEqual(actual.elements[1], value.elements[1]);
      });

      it('should support udts', async () => {
        const id = idSequence++;
        ...
        await g.addV('label_complex').property('id', id).property('prop_udt1', udt1).iterate();
        const values = await g.V().hasLabel('label_complex').has('id', id).values('prop_udt1').toList();
        assert.deepStrictEqual(values[0], udt1.value);
      });
    });
  });
});
...
function validateVertexResult(result, expectedResult, vertexLabel, propertyName) {
  ...
  helper.assertInstanceOf(prop, VertexProperty);
