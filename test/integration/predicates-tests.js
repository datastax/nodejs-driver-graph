...
const { Client, geometry } = require('cassandra-driver');
const dseGraph = require('../../index');
const { Point, Polygon, LineString } = geometry;
  ...
  context('with classic engine', () => {
    describe('predicates', function () {
      ...
      before(helper.wrapClient(function (client, done) {
        const createGraphQuery = `system.graph(name)
          .ifNotExists()
        ...
        helper.series([
          helper.toTask(
            client.executeGraph, client, createGraphQuery, { name: addressBookGraphName }, { graphName: null }),
